...
Several of Vibepollo's headline features are Windows-only, built against Win32/driver-level APIs
with no Linux equivalent. This fork maps each one to a Linux-native mechanism with the same
end-user outcome. Status reflects live verification on real hardware, not just code that compiles
— see `docs/linux/LEARNINGS.md` §24-25 for the full verification notes:

* **RTSS & NVIDIA Control Panel frame limiting → MangoHud + NVIDIA env vars.** ✅ Confirmed working
  end-to-end (MangoHud FPS cap + `__GL_SYNC_TO_VBLANK`/`__GL_MaxFramesAllowed` injected into the
  launched process). NVIDIA parity is weaker than Windows (no NVAPI-equivalent system-wide toggle).
* **Playnite Integration → Steam library sync (Lutris planned next).** ✅ Shipped end-to-end:
  library discovery, a Web UI tab, and launch/quit tracking via a companion launcher binary
  (`tools/steam_launcher/`) so quitting in-game ends the stream and vice versa — all confirmed live
  against a real Moonlight client and a Proton title.
* **Native Virtualized Display → KDE `kscreen-doctor` + EDID-injected output.** 🟡 Apply path
...
* **WebRTC browser streaming (`/webrtc`).** 🔴 Not functional on Linux yet —
  `SUNSHINE_ENABLE_WEBRTC` is Windows-only by default. Scoping is done
  (`docs/linux/webrtc-linux-port-plan.md`) and looks smaller than expected, but no Linux build of
  the `libwebrtc` dependency has been attempted yet.
...
Linux" above for details on the four features actively being ported.
...
* **Display Setting Automation**
  ...
  **Linux: 🟡 partial** — apply path is live; crash/reboot recovery isn't reproduced yet.
...
* **Windows Graphics Capture in Service Mode**
  ...
  **Linux: N/A** — different mechanism (KMS/DRM, VA-API, Vulkan, Wayland, or portal capture), not a gap.
...
* **WebRTC Browser Streaming**
  ...
  **Linux: 🔴 not functional yet** — see above.
...
* **Redesigned Frontend with Full Mobile Support**
  ...
  **Linux: ✅ works** — platform-agnostic, including the Linux-specific tabs added for this port.
...
* **Playnite Integration**
  ...
  **Linux: ✅ Steam sync shipped** as the Linux equivalent — see above.
...
* **RTSS & NVIDIA Control Panel Integration**
  ...
  **Linux: ✅ works** via the MangoHud/env-var equivalent — see above.
...
* **Frame‑Generated Capture Fixes**
  ...
  **Linux: 🔴 not yet addressed** — depends on the still-unverified virtual display revert path.
...
* **Lossless Scaling & NVIDIA Smooth Motion**
  ...
  **Linux: 🔴 not planned** — Lossless Scaling is Windows-only with no Linux build.
